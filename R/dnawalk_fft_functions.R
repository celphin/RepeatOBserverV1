...
roll_sum_histogram <- function(fname=fname, outpath=outpath){
  ...
  for (i in chromosome_list) {
    ...
    # remove the first and last 2Mbp to avoid the telomeres, remove the very small repeat lengths
    All_spec <- All_spec0[c(1:(nrow(All_spec0)-35)),c(400:(ncol(All_spec0)-400))]
...
#------------------------------
  ...
  if (length(RepeatAbund_cent)>0){
    ...
    RepeatAbund_cent$Label <- rep("RepAbundMin", nrow(RepeatAbund_cent))
  }
